...
=======
> **Nota de implementacion:** El servicio ETL (`app/services/etl_service.py`) esta definido con APScheduler pero **no se inicia automaticamente** en `main.py`. Para activar la ingesta programada (INEI, SEACE, JNE), debe conectarse `etl_service.start()` en un evento `startup` de FastAPI. Actualmente los datos de prueba se cargan via el script `app/scripts/seed_infobras.py` que se ejecuta en el entrypoint de produccion.
